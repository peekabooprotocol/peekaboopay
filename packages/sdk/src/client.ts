...
import { setPolicy, getPolicies } from "./policy";
...
export class PASClient {
	...
	/** Returns the underlying PrivacyBackend (for MCP server integration). */
	getBackend(): PrivacyBackend {
		return this.backend;
	}

	/** Returns the core engine (for advanced use cases). */
	getEngine(): PASEngine {
		return this.engine;
	}
	...
	async getPolicies(): Promise<PolicyRule[]> {
		return getPolicies(this.engine);
	}
}
